...
def raster_to_strokes(image_path, target_width=220, threshold=200, invert=False):
    """グレースケール画像を、横線セグメントの列として変換する。

    画像の黒い領域を走査して、連続した黒画素列ごとに 1 ストロークを作る。
...
def write_svg_from_image(image_path, output_svg, target_width=220, threshold=200, invert=False):
    """ローカル画像を SVG として保存する。"""
    strokes = raster_to_strokes(image_path, target_width=target_width, threshold=threshold, invert=invert)
    img = _normalize_image(image_path, target_width=target_width, threshold=threshold, invert=invert)
    width, height = img.size
    svg = [
...
def draw_image_to_board(
    ...
    strokes = raster_to_strokes(image_path, target_width=220, threshold=threshold, invert=invert)
...
def main():
    ...
    if a.svg:
        write_svg_from_image(a.image, a.svg, target_width=220, threshold=a.threshold, invert=a.invert)
